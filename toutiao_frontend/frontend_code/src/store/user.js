...
export const useUserStore = defineStore('user', {
  ...
  actions: {
    async updateUserProfile(payload) {
      ...
      try {
        const response = await axios.put(`${apiConfig.baseURL}/api/user/update`, payload, { headers: { Authorization: this.token } });
        if (response.data?.code === 200) {
          this.userInfo = response.data.data;
          return { success: true };
        }
        return { success: false, message: response.data?.message };
      } catch (error) { return { success: false, message: error.response?.data?.detail || '网络请求失败' }; }
    },
    ...
    async updateUserBio(bio) {
      try {
        ...
        // 发送更新个人简介请求
        const response = await axios.put(`${apiConfig.baseURL}/api/user/update`, { bio }, { headers: { Authorization: this.token } });
